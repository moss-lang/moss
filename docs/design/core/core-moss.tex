...
Rocq in \texttt{CoreMoss.v} alongside this document; the proofs of
Theorem~\ref{thm:sound} and Proposition~\ref{prop:phase} remain admitted
there.}}
\date{July 28, 2026}
...
\[
\begin{array}{llcl}
\text{receiver keys} & \kappa & ::= & N \mid t \\[2pt]
\text{nominal refs} & \nu & ::= & N[\theta] \\
\text{types} & \tau & ::= & \unitv \mid t \mid \This \mid
  \nu_1 \vee \cdots \vee \nu_k \quad (k \ge 0) \\
\text{applications} & \theta & ::= & [\,\ov{t = \tau}\,] \\[4pt]
\text{items} & \iota & ::= & t \mid v \mid f \mid \kappa.m[\theta] \\
...
\text{binds} & \beta & ::= &
    t = \tau \mid v = e \mid f = f' \mid \kappa.m[\theta] = f' \\[4pt]
\text{programs} & P & ::= & \ov{d}
...
\end{mathpar}
\caption{Expression typing. In \textsc{T-Meth}, $\head(N[\theta]) = N$
and $\head(t) = t$; the rule carries the side condition that the
provision at the key $(\kappa, m)$ is unique in $\Phi$. In
...
\begin{mathpar}
\inferrule[T-BindTy]{\Sigma(t) = \ksf{type} \\
...
\inferrule[T-BindMeth]{\Sigma(m) = \ksf{mth}[\Delta_m](\ov{x{:}\tau} \to \tau_r) \\
  \Sigma;\Phi \vdash \kappa \wf \\
  \Sigma;\Phi \vdash \theta \entails \Delta_m \\
  \rho = \sigma \circ \thh \circ \subst{\This \mapsto \sigma\kappa} \\
  \Sigma(f') = \ksf{fn}[\Delta']\,
    ((y_0{:}\tau_0', \ov{y{:}\tau'}) \to \tau')\,\{e_b\} \\
  \Sigma;\Phi \vdash \emptyset \entails \Delta' \\
  \sigma\tau_0' = \sigma\kappa \\
  \sigma\ov{\tau'} = \rho\ov{\tau} \\ \sigma\tau' = \rho\tau_r \\
  \Sigma;\langle A \cup \{\kappa.m[\theta]\}; \sigma \rangle;\Gamma \vdash e : \tau}
  {\Sigma;\langle A; \sigma \rangle;\Gamma \vdash
    \kw{bind}\;\kappa.m[\theta] = f';\;e : \tau}
\end{mathpar}
...
\inferrule[W-Meth]{\Sigma;\Delta_0 \vdash \Delta \wf \\
  ...
  \Sigma;\Phi_{\Delta} \vdash \kappa \wf \\
  \Sigma;\Phi_{\Delta} \vdash \theta \entails \Delta_m}
  {\Sigma;\Delta_0 \vdash \Delta, \kappa.m[\theta] \wf}
\end{mathpar}
...
receiver's type, takes its head as the key, and demands the unique
provision at $(\kappa, m)$. The provision's own bracket bindings $\theta$
...
\inferrule[E-BindMeth]{\delta[\kappa.m[\theta] \mapsto
    \clo{f'}{\delta \circ \varsigma}];\gamma \vdash e \Eval w}
  {\delta;\gamma \vdash \kw{bind}\;\kappa.m[\theta] =^{\varsigma} f';\;e \Eval w}
\end{mathpar}
...
\S\ref{sec:calculus} and the statement above are mechanized in Rocq
(\texttt{CoreMoss.v}, checked in CI), where the calculus additionally
carries an executable fueled interpreter: the program of
Figure~\ref{fig:example}, encoded in the core, computes to $\unitv$ by
\texttt{vm\_compute}, and Proposition~\ref{prop:nosearch} is proved.
The proof of the theorem itself remains admitted there. Its empirical
