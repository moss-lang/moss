...
and the metatheoretical statements of \S\ref{sec:meta} are mechanized in
Rocq in \texttt{CoreMoss.v} alongside this document; everything in
\S\ref{sec:meta} except Theorem~\ref{thm:sound} is machine-checked, and
Theorem~\ref{thm:sound} is itself derived there from a stated safety
invariant whose per-rule case lemmas remain admitted.}}
\date{July 28, 2026}
...
Moss compiles itself to a byte-identical fixpoint.
\end{abstract}
...
parameters---totally or not at all [D21, D22]. The design is validated
...
requirements. Elaborating a program yields a global signature $\Sigma$
...
for some $\iota' \in A$; comparing after $\sigma$ is the core's residue of
Moss's key canonicalization [D43].

\paragraph{Total-or-absent application.}
A reference either binds all of its target's outstanding requirements or
none of them [D22]. Given $\theta$ with $\dom(\theta)$ contained in the
type items of $\Delta$, its \emph{completion} $\thh$ extends $\theta$
with the identity on the remaining type items of $\Delta$---the
pass-through reading, under which an unapplied reference re-exports its
requirements as the very same symbols. The judgment
...
\begin{mathpar}
\inferrule[S-Nil]{ }{\Sigma;\Phi \vdash \theta \entails \emptytel}

\inferrule[S-Inst]{\Sigma;\Phi \vdash \theta \entails \Delta \\
  t \in \dom(\theta) \\ \Sigma;\Phi \vdash \theta(t) \wf}
  {\Sigma;\Phi \vdash \theta \entails \Delta, t}

\inferrule[S-Pass]{\Sigma;\Phi \vdash \theta \entails \Delta \\
  t \notin \dom(\theta) \\ t \in \Phi}
  {\Sigma;\Phi \vdash \theta \entails \Delta, t}

\inferrule[S-Item]{\Sigma;\Phi \vdash \theta \entails \Delta \\
  \iota \text{ not a type symbol} \\ \thh\,\iota \in \Phi \\
  \sigma(\thh\,t) = \sigma(t) \;\;(\forall t \in \ksf{deps}(\iota))}
  {\Sigma;\Phi \vdash \theta \entails \Delta, \iota}
\end{mathpar}
...
\inferrule[Sub-Inj]{\sigma\nu = \sigma\nu_j \text{ for some } j}
  {\Sigma;\Phi \vdash \nu \le \nu_1 \vee \cdots \vee \nu_k}
\end{mathpar}
\caption{Satisfaction, type formation, and subtyping. In
\textsc{S-Inst}/\textsc{S-Pass}/\textsc{S-Item}, $\dom(\theta)$ must be
contained in the type items of the full telescope; $\sigma$ is the
...
\inferrule[T-Meth]{\Sigma(m) = \ksf{mth}[\Delta](\ov{x{:}\tau} \to \tau_r) \\
  ...
  \sigma(\thh(\sigma\tau')) = \sigma(\thh\,\tau') \\
  \rho = \sigma \circ \thh \circ \subst{\This \mapsto \sigma\tau_0} \\
...
is unique in $\Phi$; keying on the head alone is adequate under Moss's
unique-nominal-per-instantiation idiom [D51] but unsound in the raw
calculus, a repair owed to the mechanization. In
...
\inferrule[T-BindMeth]{\Sigma(m) = \ksf{mth}[\Delta_m](\ov{x{:}\tau} \to \tau_r) \\
  ...
  \sigma(\thh(\sigma\tau_\kappa)) = \sigma(\thh\,\tau_\kappa) \\
  \rho = \sigma \circ \thh \circ \subst{\This \mapsto \sigma\tau_\kappa} \\
...
\quad (write $\Phi_{\Delta} = \langle \Delta_0 \cup \Delta;\, \emptyset \rangle$)
\begin{mathpar}
...
satisfy each requirement, and typing fails rather than choosing.
\end{proposition}
...
weakening lemma for $\delta \circ \varsigma$. The definitions of
\S\ref{sec:calculus} and all of the statements here are mechanized in
Rocq (\texttt{CoreMoss.v}, checked in CI), which additionally carries an
executable fueled interpreter: the program of Figure~\ref{fig:example},
encoded in the core, computes to $\unitv$ by \texttt{vm\_compute}.
Proposition~\ref{prop:nosearch}, determinism of evaluation, adequacy of
the interpreter for the big-step relation in both directions, and
Proposition~\ref{prop:phase} are machine-checked outright.  The theorem
above is derived in the mechanization from a single stated safety
invariant --- fuel-indexed, with a ground instantiation of the abstract
type symbols carried per closure --- whose per-typing-rule case lemmas
remain admitted; designing that invariant is what surfaced the three
repairs below. Its empirical
...
Three repairs, all surfaced while designing the preservation invariant,
are already folded into the figures above. (i)~Dispatch must key on the
...
(v)~Method provisions need receiver coherence
(\textsc{T-BindMeth}/\textsc{T-Meth}): the application $\theta$ must
read the receiver the same over and under $\sigma$, or $\theta$ captures
a symbol $\sigma$ introduced and the provider's signature is
interpreted at a different receiver than the call's --- the method-item
twin of \textsc{S-Item}'s coherence premise, likewise mechanically
refuted before it was a premise.
...
recursively. Duplicate keys \emph{merge} by unification, erring only
when two distinct concrete types would be identified [D19, D43]. The
core requires keys distinct and keeps only the $\sigma$-canonicalized
membership test. \\[3pt]
Functors &
...
specialization. The consistent-merging rule for contexts [D43], omitted
from the core, is a deterministic congruence over requirement sets;
